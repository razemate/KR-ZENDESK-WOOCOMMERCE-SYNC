  ...
  try {
    ...
    while (true) {
      ...
      for (const sub of enrichedSubs) {
        ...
        // Apply the fetched latest order
        if (sub._latest_order_fetched) {
             const latest = sub._latest_order_fetched;
             row.latest_order_id = latest.id;
             row.latest_order_admin_url = `${process.env.WOO_BASE_URL?.replace(/\/+$/,"")}/wp-admin/post.php?post=${latest.id}&action=edit`;
             row.latest_order_status = (latest.status || "").toString() || null;
             row.latest_order_date_iso = (latest.date_created_gmt || latest.date_created || "").toString() || null;
        }
